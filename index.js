...
io.on('connection', async (socket) => {
    ...
    socket.on('req_guestSide', () => {
        ...
        socket.on('disconnect', () => {
            ...
            if (qrCode) {
                console.log(`Deleted QR code ${qrCode} associated with socket ${socket.id}`);
            }
        });
    });
        ...
        if (qrCodeData) {
            ...
            if (session) {
                ...
                if (user && user.roleId == 2) {
                    ...
                    io.to(socketId).emit('qrCode_hasRead', { authGuestCode: sessionCode, shopId: user.cafeId });
                    socket.emit('qrCode_readSuccess', { shopId: user.cafeId });

                    // Create guest side session and store it
                    const guestSideSession = [session.userId, socket.id, socketId, sessionCode, user.cafeId];
                    ...
                    console.log(`Deleted QR code ${qrCode} after reading by client ${socketId}`);
                }
            }
        } else {
    ...
    socket.on('checkGuestSideToken', async (data) => {
        ...
        if (!token) {
            return socket.emit('checkGuestSideTokenRes', { status: 404, message: 'token is required' });
        }
        ...
        if (!sessionData) {
            return socket.emit('checkGuestSideTokenRes', { status: 404, message: 'Guest side session not found or invalid' });
        }
        ...
        socket.emit('checkGuestSideTokenRes', { status: 200, message: 'Guest side session verified successfully', sessionData });
    });
...
app.post('/getConnectedGuestsSides', async (req, res) => {
    ...
    if (sessionDatas.length < 1) {
        return res.status(404).json({ error: 'Guest side session not found or invalid' });
    }
    ...
    // Respond with success or additional data if needed
    res.status(200).json({ message: 'getting Guest side session successfully', sessionDatas });
});
...
app.post('/removeConnectedGuestsSides', async (req, res) => {
    ...
    const rm = clerkHelper.deleteGuestSideSessionByGuestSideSessionId(session.userId, req.body.guestSideSessionId);
    if(!rm) return res.status(404).json({ error: 'side session not found or invalid' });
    
    io.sockets.sockets.get(rm.previousGuestSideSocket).emit('signout-guest-session');
    res.status(200).json({ message: 'removing Guest side session successfully', guestSideList: rm.guestSideList });
});
...
setInterval(async () => {
    for (const shopId in spotifyService.rooms) {
        ...
        if (!spotifyService.rooms[shopId].paused) {
            if (((playbackState && !playbackState.is_playing && playbackState.currently_playing_type === 'track'))) {
                ...
                if (playbackState.item && playbackState.item.duration_ms) {
                    ...
                    if (remainingTime <= 40000 && remainingTime > 12000 && spotifyService.rooms[shopId].nextTrack === "") {
                        ...
                        const subscriptionStatus = await spotifyService.getUserSubscriptionStatus(shopId);
        ...
        if (playbackState?.is_playing) {
            ...
            const lyrics = await spotifyService.fetchRandomTrackLyrics(shopId, playbackState.item.id, true);
            console.log('force');
